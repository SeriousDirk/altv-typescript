...
let cmdHandlers = {
    weapon(player: alt.Player, weaponName: string, ammo: number) {
        player.giveWeapon(alt.hash(weaponName), ammo, false);
    },
    vehicle(player: alt.Player, vehicleName: string) {
        const { x, y, z } = player.pos;
        const spawnedVeh = new alt.Vehicle(alt.hash(vehicleName), x, y, z, 0, 0, 0);
        spawnedVeh.customPrimaryColor = new alt.RGBA(255, 100, 100, 255);
